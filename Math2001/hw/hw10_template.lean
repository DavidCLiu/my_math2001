...
theorem uniqueness (a b : ℤ) (h : 0 < b) {r s : ℤ}
    ...
    dsimp [Int.ModEq] at hr3
    dsimp [Int.ModEq] at hs3
    dsimp [(· ∣ ·)] at hr3
    dsimp [(· ∣ ·)] at hs3
    obtain ⟨c, hc⟩ := hr3
    obtain ⟨d, hd⟩ := hs3
...
theorem problem4b (a b : ℤ) (h : 0 < b) : ∃! r : ℤ, 0 ≤ r ∧ r < b ∧ a ≡ r [ZMOD b] := by
  ...
  . constructor
    ...
    . constructor
      ...
      . use fdiv a b
        have Hab : fmod a b + b * fdiv a b = a := by apply fmod_add_fdiv a b
        addarith [Hab]
  . intro r hr
...
theorem problem5a (a b : ℤ) : gcd a b ∣ b ∧ gcd a b ∣ a := by
  ...
  · -- case `0 < b`
    ...
    · -- prove that `gcd a b ∣ a`
      have IH1 := IH_left  -- inductive hypothesis
      have IH2 := IH_right  -- inductive hypothesis
      obtain ⟨k, hk⟩ := IH1
      obtain ⟨l, hl⟩ := IH2
...
theorem problem5b {d a b : ℤ} (ha : d ∣ a) (hb : d ∣ b) : d ∣ gcd a b := by
  ...
  obtain ⟨k, hk⟩ := ha
  obtain ⟨l, hl⟩ := hb
  have h4 : d ∣ x * a + y * b := by
    use x * k + y * l
    calc
      x * a + y * b = x * (d * k )+ y * (d * l) := by rw[hk,hl]
      _ = d * (x * k + y * l) := by ring
  rw [h2] at h4
  apply h4
